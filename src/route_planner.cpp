...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    // Use the FindNeighbors() method of the current_node to populate current_node.
    // neighbors vector with all the neighbors.
...
RouteModel::Node *RoutePlanner::NextNode() {
    ...
    // The last element of open_list is our target since the elements are sorted 
    // in descending order Create a pointer to the node in the list with the lowest sum
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    auto isStartNodeReached = [&](const RouteModel::Node *inode){
        return (inode->x == start_node->x && inode->y == start_node->y);
    };
    ...
    // The returned vector should be in the correct order: the start node should be the 
    // first element
...
void RoutePlanner::AStarSearch() {
    ...
    auto isEndNodeReached = [&](const RouteModel::Node *inode){
        return (inode->x == end_node->x && inode->y == end_node->y);
    };
